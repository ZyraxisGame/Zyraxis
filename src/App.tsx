...
export default function App() {
  ...
  // Faction-aware exposure display threshold
  ...
  return (
    <div style={{ fontFamily: "'Courier New',Courier,monospace", background: '#030508', color: '#e5e7eb', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {/* Header */}
      <div style={{ borderBottom: '1px solid #0d1018', padding: '7px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#030508', flexShrink: 0 }}>
        <div>
          <div style={{ fontSize: 6, letterSpacing: 5, color: '#141c28' }}>CLASSIFIED — RESTRICTED ACCESS — LEVEL OMEGA</div>
          <div style={{ fontSize: 13, letterSpacing: 5, color: faction?.accent || '#818cf8', animation: 'flicker 9s infinite, glow 4s infinite' }}>ZYRAXIS</div>
        </div>
        <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
          <div style={{ fontSize: 7, color: '#1f2937', letterSpacing: 2, textAlign: 'right', lineHeight: 1.8 }}>
            <div>TICK {String(tick).padStart(4, '0')}</div>
            <div style={{ color: res.exposure >= expDisplayThreshold ? '#f97316' : '#1f2937' }}>
              EXP {res.exposure || 0}%
            </div>
          </div>
          <button onClick={reset} disabled={executing}
            style={{ fontSize: 6, letterSpacing: 2, background: 'transparent', border: '1px solid #141c28', color: executing ? '#1f2937' : '#374151', cursor: executing ? 'not-allowed' : 'pointer', padding: '3px 9px' }}>
            RESET
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '18px 14px' }}>
          <div style={{ fontSize: 6, letterSpacing: 4, color: '#141c28', marginBottom: 12 }}>[ THE VEIL — POWER HIERARCHY — SUPREME FORCE AT TOP ]</div>
          {tiersOrdered.map(t => (
            <TierCard
              key={t.id}
              tier={t}
          ...
          <div style={{ marginTop: 16, fontSize: 7, color: '#141c28', letterSpacing: 1, lineHeight: 2 }}>
        ...
        {/* Op Log */}
        <div style={{ width: 180, borderLeft: '1px solid #0d1018', padding: '12px 10px', background: '#030508', overflowY: 'auto', flexShrink: 0 }}>
          <div style={{ fontSize: 6, letterSpacing: 4, color: '#141c28', marginBottom: 9 }}>[ OP LOG ]</div>
          {log.map((entry, i) => (
            <div key={i} style={{ fontSize: 7, color: i === 0 ? '#4b5563' : '#141c28', marginBottom: 5, lineHeight: 1.5, borderLeft: i === 0 ? `2px solid ${faction?.accent || '#1f2937'}` : 'none', paddingLeft: i === 0 ? 5 : 0 }}>
              {entry}
            </div>
          ))}
        </div>
      ...
      {/* Reinvest prompt — Silent Economy */}
      {pendingReinvest && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', zIndex: 120, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#07090f', border: '1px solid #34d39940', maxWidth: 420, width: '90%', padding: 28 }}>
            ...
            <div style={{ fontSize: 7, letterSpacing: 4, color: '#34d399', marginBottom: 6 }}>[ REINVESTMENT AVAILABLE ]</div>
            <div style={{ fontSize: 10, color: '#e5e7eb', marginBottom: 8 }}>Standard bonus already applied. Reinvest to compound your gains?</div>
            <div style={{ fontSize: 8, color: '#4b5563', lineHeight: 1.6, marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={confirmReinvest}
                style={{ flex: 1, padding: '8px', fontSize: 9, letterSpacing: 2, background: '#34d399', color: '#000', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
                REINVEST
              </button>
              <button onClick={dismissReinvest}
                style={{ flex: 1, padding: '8px', fontSize: 9, letterSpacing: 2, background: 'transparent', color: '#4b5563', border: '1px solid #1a2030', cursor: 'pointer', fontFamily: 'inherit' }}>
                TAKE STANDARD
              </button>
            </div>
          </div>
